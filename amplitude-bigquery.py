...
ACCOUNT_ID = '234624'
API_KEY = '9eaf49286fab3eb7194388494974bfc0'
API_SECRET = '166cd28178ef7f024dd0e54f779aca74'
PROJECT_ID = 'flick-267310'
CLOUD_STORAGE_BUCKET = 'flick-amplitude-events'
...
START = (datetime.utcnow().date() - timedelta(days=194)).strftime("%Y%m%d")
END = (datetime.utcnow().date() - timedelta(days=184)).strftime("%Y%m%d")

# Perform a CURL request to download the export from Amplitude
os.system("curl -u " + API_KEY + ":" + API_SECRET + " \
          'https://amplitude.com/api/2/export?start=" + START + "T01&end="
          + END + "T23'  >> amplitude.zip")

# Unzip the file
unzip_file('amplitude.zip', 'amplitude')

# Initiate Google BigQuery
bigquery_client = bigquery.Client.from_service_account_json("./bigquery.json")

# Initiate Google Cloud Storage
storage_client = storage.Client.from_service_account_json("./bigquery.json")
...
for file in file_list('.gz'):
    ...
    print("{}".format(PATH + file_json(file)))
    ...
    load_into_bigquery(file, 'events$' + START)
